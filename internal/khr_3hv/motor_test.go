...
func TestYaml(t *testing.T) {
	...
	r := Robot{}
	if err := r.LoadIDWithYaml(tt); err != nil {
		t.Fatal(err)
	}
	var id uint8
	id = 0
...
func TestDefaultRobotNum(t *testing.T) {
	r := RobotNum{}
	settingRobotNumID(&r)
	...
	id = 3
	if r[LeftElbowYaw].GetID() != id {
		t.Errorf("ID LeftElbowYaw is wrong, should be %d, but actual %d", id, r[RightElbowRoll].GetID())
	}
	if r[RightElbowYaw].GetID() != id {
		t.Errorf("ID RightElbowYaw is wrong, should be %d, but actual %d", id, r[RightElbowYaw].GetID())
	}
	id = 4
	if r[LeftElbowRoll].GetID() != id {
		t.Errorf("ID LeftElbowRoll is wrong, should be %d, but actual %d", id, r[LeftElbowRoll].GetID())
